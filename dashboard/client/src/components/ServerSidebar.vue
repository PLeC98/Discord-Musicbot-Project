<template>
  ...
  <aside class="hidden md:flex shrink-0 sticky top-[var(--chrome)] h-[calc(100dvh_-_var(--chrome)_-_var(--player))] flex-col border-r border-white/7 bg-[rgba(7,11,21,0.45)] backdrop-blur-[20px] transition-[width] duration-300 ease-smooth overflow-hidden" :class="collapsed ? 'w-16' : 'w-16 lg:w-60'">
  ...
  <div class="lg:hidden fixed inset-x-0 top-[calc(var(--chrome)_+_3rem)] md:top-[var(--chrome)] bottom-[var(--player)] z-140 bg-black/55 transition-opacity duration-300" :class="drawerOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'" @click="closeDrawer"></div>

  <aside class="lg:hidden fixed left-0 top-[calc(var(--chrome)_+_3rem)] md:top-[var(--chrome)] bottom-[var(--player)] z-150 w-60 flex flex-col bg-[rgba(7,11,21,0.92)] backdrop-blur-2xl backdrop-saturate-[1.8] border-r border-white/10 transition-transform duration-300 ease-smooth" :class="drawerOpen ? 'translate-x-0' : '-translate-x-full'">
