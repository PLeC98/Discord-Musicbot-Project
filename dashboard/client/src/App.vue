<template>
  ...
  <div :class="[user.viewAs ? '[--chrome:1.75rem]' : '[--chrome:0px]', nowPlaying.visible ? '[--player:3.5rem] md:[--player:4rem]' : '[--player:0px]', railVars]">
    ...
    <header v-if="user.isLoggedIn" class="md:hidden flex items-center px-2 h-12 gap-1 sticky top-(--chrome) z-100 bg-[rgba(7,11,21,0.72)] backdrop-blur-2xl backdrop-saturate-[1.8] border-b border-white/7">
      <button class="size-9 shrink-0 rounded-lg text-fg-soft cursor-pointer flex items-center justify-center transition-[background-color,color] duration-200 hover:bg-white/6 hover:text-fg" v-tooltip="'사이드바'" @click="toggleSidebar">
    ...
    <div class="flex items-start">
      ...
      <main class="flex-1 min-w-0 pb-(--player)">
