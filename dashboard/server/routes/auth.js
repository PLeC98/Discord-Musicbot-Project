...
log.info({ sub: "auth" }, `리디렉션 URI: ${REDIRECT_URI}`);
log.info({ sub: "auth" }, `운영자 ID: ${OWNER_ID ? "설정됨" : "미설정"}`);
...
router.get("/callback", async (req, res) => {
  ...
  try {
    ...
    log.error("❌ OAuth 콜백 오류:");
    log.error("  상태 코드:", error.response?.status);
    log.error("  응답 본문:", JSON.stringify(discordErr));
    log.error("  사용한 리디렉션 URI:", REDIRECT_URI);
