...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const embed = new EmbedBuilder()
      .setTitle("🎧 DJ 역할 설정")
      .setDescription([statusLine, "", "아래 메뉴에서 DJ로 지정할 역할을 고른 뒤 **저장**을 누르세요. (복수 선택 가능)", "아무 역할도 선택하지 않고 저장하면 제한이 해제됩니다.", "모더레이터(서버 관리·추방·차단·타임아웃 권한 보유자)는 설정과 무관하게 항상 제어할 수 있어요."].join("\n"))
      .setColor("#5865F2");
    ...
    await interaction.reply({
      ...
      components: [
        ...
        new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("djrole:save").setLabel("저장").setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId("djrole:cancel").setLabel("취소").setStyle(ButtonStyle.Secondary)),
      ],
