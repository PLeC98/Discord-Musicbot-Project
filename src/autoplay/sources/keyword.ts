...
/** 유튜브 검색. 결과에서 여기서 읽는 칸만 */
type Search = (query: string, limit: number) => Promise<Array<{ id?: string | number | null; audioUrl?: string | null; title: string; duration?: number; isLive?: boolean; thumbnail?: string | null }> | null | undefined>;
...
async function keyword(source: GenreSource, { search = (query, limit) => YouTube.search(query, limit) }: { search?: Search } = {}): Promise<Candidate[]> {
  ...
  return results.flatMap((r) => (r.audioUrl && !r.isLive ? [{ title: r.title, durationSec: r.duration, youtubeUrl: r.audioUrl, thumbnail: r.thumbnail, fromSearch: true, sourceKey: `yt:${r.id}` }] : []));
}
