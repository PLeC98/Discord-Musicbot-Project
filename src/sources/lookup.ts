 ...
 */
function isUnsupportedLink(query) {
...
async function getTrackData(query, context = "lookup.getTrackData", { offset = 0, limit } = {}) {
  try {
    let tracks = [];
...
async function getCollection(url, range) {
 ...
 */
async function resolveQuery(query, context, range = {}) {
