 ...
 */
async function _fetchRaw(videoId) {
 ...
 */
async function lookup(videoId, { categories = config.sponsorblock.categories } = {}) {
...
async function _rawFor(videoId) {
 ...
 */
async function forTrack(track, guildId) {
...
function _remember(videoId) {
  const entry = { at: Date.now(), none: false, promise: null };
  entry.promise = _rawFor(videoId)
