...
class ExternalCaches {
  ...
  getSponsorSegments(videoId: string | null | undefined): { segments: unknown; fetchedAt: number } | null {
    if (!videoId) return null;
    const row = this.db.prepare("SELECT data_json, fetched_at FROM sponsorblock_cache WHERE video_id = ?").get(videoId) as JsonRow | undefined;
    if (!row) return null;
    try {
  ...
  setSponsorSegments(videoId: string | null | undefined, segments: unknown) {
  ...
  markAgeRestricted(videoId: string | null | undefined) {
    if (!videoId) return;
  ...
  isAgeRestricted(videoId: string | null | undefined) {
  ...
  getSpotifyAnonState(): (Record<string, unknown> & { fetchedAt: number }) | null {
    const row = this.db.prepare("SELECT data_json, fetched_at FROM spotify_anon WHERE id = 1").get() as JsonRow | undefined;
    if (!row) return null;
    try {
  ...
  setSpotifyAnonState(data: unknown) {
