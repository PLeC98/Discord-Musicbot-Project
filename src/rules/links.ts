...
function isYouTubeHost(value: unknown): boolean {
...
function isYouTubeURL(value: unknown): boolean {
...
function isYouTubePlaylist(value: unknown): boolean {
  const parsed = parseYouTubeURL(value);
...
function isSpotifyURL(url: unknown): boolean {
  ...
  return patterns.some((p) => p.test(String(url)));
}

function parseSpotifyURL(url: unknown): { type: string | null; id: string | null } {
...
function isSoundCloudURL(url: unknown): boolean {
  const patterns = [
...
function isDirectAudioLink(url: unknown): boolean {
  try {
...
function isHttpLink(value: unknown): boolean {
