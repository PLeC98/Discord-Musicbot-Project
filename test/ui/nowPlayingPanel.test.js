...
function memoryStore(initial = {}) {
  ...
  return {
    ...
    setPanel: async (g, channelId, messageId) => records.set(g, messageId ? { channelId, messageId } : null),
  };
}

function fakeWebhook(log, { failDelete = false } = {}) {
  return {
...
function fakeChannel(id, log) {
  return { id, messages: { delete: async (messageId) => log.push(`channel:${id}:${messageId}`) } };
}

function setup({ record, webhooks = {} } = {}) {
  const log = [];
  ...
  const embeds = { getOrCreateWebhook: async (channel) => webhooks[channel.id] ?? null };
