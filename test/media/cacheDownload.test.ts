...
import { TrackDownloader, type DownloadTrack, type Sources } from "../../src/media/cacheDownload.ts";
...
// 시험이 트랙에 달아 두는 것: 동등물을 찾으면 줄 영상
type TestTrack = DownloadTrack & { _equivalent?: string };
// 받기는 언제나 출력 경로를 준다
type Options = YtDlpFlags & { output: string };

// 무엇이 불렸는지 모은다
const calls = {
  ytdlp: [] as Array<{ url: string; options: Options }>,
  direct: [] as string[],
  convert: [] as Array<{ src: string; out: string; srcExists: boolean }>,
  sponsor: [] as Array<{ title: string; guildId: string }>,
  equivalent: [] as string[],
  reresolve: [] as string[],
};
// 시험마다 바꾸는 yt-dlp 의 행동. 기본은 "출력 경로에 파일과 info.json 을 쓴다"
let ytdlpBehavior: (url: string, options: Options) => unknown;
...
const deps: Sources = {
  youtube: {
    ...
    runYtDlp: async (url, build) => {
      const flags = build(false);
      assert.ok(flags.output, "출력 경로가 없다");
      const options = { ...flags, output: flags.output };
  ...
  convert: {
    ...audioConvert,
    toCacheOpus: async (src: string, out: string) => {
...
test("유튜브: yt-dlp 로 임시 파일에 받아 최종 경로로 올리고 장부에 적는다", async () => {
  ...
  assert.ok(Number(row.version_checked_at) > 0);
  const lookup = lookupRow(track.requestKey);
  assert.ok(lookup, "요청을 적었다");
...
test("스포티파이: 동등물을 못 찾으면 받기 전에 실패한다. 아무것도 적거나 남기지 않는다", async () => {
  ...
  assert.equal(storeDb.get().prepare<[], { n: number }>("SELECT COUNT(*) AS n FROM audio_cache").get()?.n, 0);
...
test("자동재생 곡은 장부에 요청 열쇠(소스 안의 곡)로 적힌다. 같은 작품의 두 곡이 따로 산다", async () => {
  ...
  const song = (id: number, vid: string) => ({ title: `곡 ${id}`, pageUrl: page, requestKey: `amq:${id}`, platform: "anisongdb", audioUrl: `https://www.youtube.com/watch?v=${vid}` });
...
test("직접 링크: SafeUrl 을 거쳐 원본을 받고, 변환하고, 실측 길이로 트랙을 고친다", async () => {
  const track: DownloadTrack = { title: "파일", pageUrl: "https://files.test/a.flac", requestKey: "https://files.test/a.flac", audioUrl: "https://files.test/a.flac", platform: "direct", duration: 100 };
...
test("장부에서 가져온 영상이 내려갔으면 다시 찾아 한 번 더 받는다", async () => {
  ...
  const track: DownloadTrack = { title: "스포티파이 곡", pageUrl: "https://open.spotify.com/track/sp3", requestKey: "https://open.spotify.com/track/sp3", platform: "spotify", audioUrl: "https://www.youtube.com/watch?v=deaddeaddea", audioFoundBy: "ledger" };
...
test("새로 검색한 영상(장부에서 온 것이 아님)이 내려갔으면 다시 찾지 않는다", async () => {
  ...
  const track: DownloadTrack = { title: "곡", requestKey: "https://open.spotify.com/track/sp4", platform: "spotify", audioUrl: "https://www.youtube.com/watch?v=nnnnnnnnnnn", audioFoundBy: "search" };
