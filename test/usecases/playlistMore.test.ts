...
test("custom_id: 되돌리면 같고, 가장 긴 조합도 100자 안이며, 깨진 값은 거절한다", () => {
  const s = { kind: "ytp", listId: `OLAK5uy_${"a".repeat(33)}`, offset: 99950, anchorId: "dQw4w9WgXcQ", insertFirst: true, requesterId: "12345678901234567890" };
...
test("메뉴: 자리가 없으면 메뉴를 붙이지 않고, 있으면 선택지 뒤에 직접 입력", () => {
  const more = { kind: "spp", listId: PL, offset: 50, anchorId: TRACK, insertFirst: false, requesterId: null, total: 9946, remaining: 9896 };
...
test("누를 때: 전용 채널 메뉴는 넣은 사람만, 30초가 지나면 닫힌다", () => {
  ...
  assert.match(More.clickError({ requesterId: null }, { userId: "a", lastTouched: now - More.LIFETIME_MS - 1, now }), /시간이 지나/);
