...
- **장르 자동재생**: 선택한 장르의 곡을 자동 탐색·재생. 재생 중에도 다음 곡을 미리 받아 둬 곡 사이가 비지 않음. `config/genres.yaml`에서 장르, 검색 키워드, 메타데이터 소스를 직접 설정 가능하며, 해당 설정은 핫 리로드 가능.
  - 사용 가능한 소스: Last.fm, ListenBrainz Radio, AnimeThemes, VocaDB, UtaiteDB, TouhouDB, 스포티파이/유튜브 재생목록. 섞어서 사용 가능, 소스별 가중치 설정 가능.
...
`.env`의 `COOKIES_SOURCE` 한 칸으로 정합니다.

| 값                                         | 동작                                                      |
| ------------------------------------------ | --------------------------------------------------------- |
| (비움)                                     | 쿠키를 쓰지 않습니다. 연령 제한 영상은 재생할 수 없습니다 |
| `chrome` · `firefox` · `edge` · `safari` … | 그 브라우저에서 직접 가져옵니다                           |
| `file`                                     | `config/cookies.txt`를 씁니다                             |

`file`로 두면 **대시보드 운영자 화면에 「유튜브 쿠키」 탭이 생깁니다.** 브라우저 확장에서 복사한 내용을 붙여넣어 갈아 끼울 수 있습니다. 쿠키는 수시로 무효가 되므로(아래 참고) 그때마다 서버에 들어갈 일을 줄여 줍니다. 저장된 값은 화면으로 다시 내려오지 않습니다.
