...
- **봇 전용 채널**: `/setchannel`로 지정한 채널에 곡명/링크만 입력하면 자동 재생. 재생 패널이 늘 채널 맨 아래에 떠 있고, 곡이 끝나면 같은 자리에서 대기 모양으로 바뀜
- **패널은 서버당 하나**: 다시 재생하면 지난 패널을 치우고 새 패널을 올림 (봇을 다시 켜도 기록으로 찾아 정리)
