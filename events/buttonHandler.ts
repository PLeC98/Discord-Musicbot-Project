import { Events, EmbedBuilder, ActionRowBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, type ButtonBuilder, type ButtonInteraction, type Client } from "discord.js";
...
/** 서버 메시지의 버튼 */
type Press = ButtonInteraction<"cached">;
...
const FREE: Array<[prefix: string, ((it: Press) => unknown) | null]> = [
  ["djrole:", null],
  ["sb:", null],
  ["search_", (it) => handleSearchInteraction(it, it.client)],
  ["help_refresh", (it) => handleHelpRefresh(it)],
  ["system_refresh", (it) => handleSystemRefresh(it)],
  ["music_autoplay:", (it) => handleAutoplayButton(it)],
];

// 재생 패널의 버튼. customId 는 "이름:요청자:세션"
const PANEL: Record<string, (it: Press, player: MusicPlayer) => unknown> = {
  music_pause: (it, player) => handlePause(it, player),
  music_skip: (it, player) => handleSkip(it, player),
  music_stop: (it, player) => handleStop(it, player),
  music_queue: (it, player) => handleQueue(it, player),
  music_shuffle: (it, player) => handleShuffle(it, player),
  music_highlight: (it, player) => handleHighlight(it, player),
  music_volume: (it) => handleVolumeModal(it),
  music_loop: (it, player) => handleLoop(it, player),
  music_previous: (it, player) => handlePrevious(it, player),
};

// 재생 패널의 버튼. 플레이어가 있고 패널의 세션이 지금 세션이어야 한다.
// 재적 · 계층 검사는 처리기가 부르는 controls 가 한다. 대기열 보기는 검사 없이 연다
async function handlePanelButton(interaction: Press) {
  ...
  try {
    ...
    await handle(interaction, player);
  } catch (error) {
    log.error(`패널 버튼 처리 실패(${interaction.customId}):`, error);
    if (interaction.replied || interaction.deferred) return;
    await interaction.reply({ content: S.ERR_PROCESSING, flags: [1 << 6] }).catch((replyError) => log.warn(`오류 안내 전송 실패: ${messageOf(replyError)}`));
  }
}

async function handlePause(interaction: Press, player: MusicPlayer) {
...
async function handleSkip(interaction: Press, player: MusicPlayer) {
...
async function handlePrevious(interaction: Press, player: MusicPlayer) {
...
async function handleStop(interaction: Press, player: MusicPlayer) {
...
async function handleQueue(interaction: Press, player: MusicPlayer) {
...
async function handleShuffle(interaction: Press, player: MusicPlayer) {
...
async function handleHighlight(interaction: Press, player: MusicPlayer) {
  ...
  if (!r.ok && r.code === "no-highlight") return interaction.deferUpdate();
  if (!r.ok) return interaction.reply({ content: controlMessage(r), flags: [1 << 6] });
  await interaction.editReply({ content: "✨ 하이라이트 지점으로 이동했어요." });
}

async function handleVolumeModal(interaction: Press) {
  ...
  const actionRow = new ActionRowBuilder<TextInputBuilder>().addComponents(volumeInput);
  modal.addComponents(actionRow);

  await interaction.showModal(modal);
}

async function handleLoop(interaction: Press, player: MusicPlayer) {
...
async function handleAutoplayButton(interaction: Press) {
  ...
  return await handleAutoplay(interaction, player, member.id);
}

// 권한은 handleAutoplayButton이 이미 봤다.
async function handleAutoplay(interaction: Press, player: MusicPlayer, requesterId: string) {
  if (player.autoplay) {
...
/** 새로 그린 화면 */
type Built = { embed: EmbedBuilder; row: ActionRowBuilder<ButtonBuilder> };

// 새로고침 버튼: 버튼이 달린 메시지를 새 내용으로 고친다
async function refreshMessage(interaction: Press, build: (client: Client<true>) => Built | Promise<Built>, { failed, notice }: { failed: string; notice: string }) {
  try {
...
function handleHelpRefresh(interaction: Press) {
  return refreshMessage(interaction, (client) => buildHelpEmbed(client), { failed: "명령어 도움말 새로고침 실패:", notice: "❌ 도움말을 새로고침하는 중 오류가 발생했습니다!" });
}

async function handleSystemRefresh(interaction: Press) {
  ...
  return refreshMessage(interaction, (client) => buildSystemEmbed(client), { failed: "시스템 정보 새로고침 실패:", notice: "❌ 시스템 정보를 새로고침하는 중 오류가 발생했습니다!" });
}

async function handleSearchInteraction(interaction: Press, client: Client<true>) {
...
const exported: ClientEvent<Events.InteractionCreate> = {
  ...
  async execute(interaction) {
    ...
    if (route) return route[1]?.(interaction);
